...
function getDeviceId(){
  ...
  if(!id){
    ...
    id="AURA-"+Math.abs(h).toString(16).toUpperCase();
...
function speak(text,whisper=false){
  ...
  u.pitch=whisper?0.85:0.75;
  u.rate=whisper?1.0:0.9;
  u.volume=whisper?0.6:1;
  const trySpeak=()=>{
    const vs=window.speechSynthesis.getVoices();
    const male=vs.find(v=>
    ...
    <div style={{width:"100%",minHeight:"100vh",background:"#010a0f",fontFamily:"'Courier New',monospace",color:"#00d4ff",position:"relative",overflow:"hidden",userSelect:"none"}}>
      ...
      <div style={{display:"flex",borderBottom:"1px solid rgba(0,212,255,0.1)",background:"rgba(0,8,18,0.95)",overflowX:"auto"}}>
        {[["home","🏠","HOME"],["reminders","📋","TASKS"],["alarm","⏰","ALARM"],["vault","🔒","VAULT"],["learn","🧠","LEARN"]].map(([id,icon,label])=>(
          <button key={id} onClick={()=>setActiveTab(id)} style={{flex:1,minWidth:64,padding:"9px 4px",background:activeTab===id?"rgba(0,212,255,0.08)":"transparent",border:"none",borderBottom:activeTab===id?"2px solid #00d4ff":"2px solid transparent",color:activeTab===id?"#00d4ff":"rgba(0,212,255,0.3)",fontSize:8,letterSpacing:1,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
